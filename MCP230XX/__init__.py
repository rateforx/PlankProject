...
class MCP230XX:
    ...
    def __init__( self, i2cAddress, chip = 'MCP23017', regScheme = '16bit' ):
        ...
        self.callBackFuncts = [ ]
        for i in range( 0, 17 ):
            self.callBackFuncts.append( [ self.__blank( ), self.__blank( ) ] )
    ...
    def clearInterrupts( self ):
        if self.bank == '16bit':  # bank = 0 in datasheet
            self.single_access_read( 0x10 )  # INTCAPA
            self.single_access_read( 0x11 )  # INTCAPB
            self.single_access_read( 0x12 )  # GPIOA
            self.single_access_read( 0x13 )  # GPIOB
        else:  # self.bank == '8bit' # bank = 0 in datasheet
            self.single_access_read( 0x08 )  # INTCAPA
            self.single_access_read( 0x09 )  # GPIOA
            self.single_access_read( 0x18 )  # INTCAPB
            self.single_access_read( 0x19 )  # GPIOB

    def add_interrupt( self, pin, callbackFunctLow = __blank, callbackFunctHigh = __blank ):
    ...
    def remove_interrupt( self, pin ):
        ...
        self.callBackFuncts[ pin ][ 0 ] = self.__blank
        self.callBackFuncts[ pin ][ 1 ] = self.__blank
    ...
    def callbackA( self, gpio ):
        ...
        if self.callBackFuncts[ pin ][ value ] != self.__blank:
...
class MCP23018( MCP230XX ):
    ...
    """If interrupt pin other than -1, the pins will be all set to input and the interrupt handling will be set up"""

    def __init__( self, i2cAddress, mode = -1 ):
        ...
        if mode != -1:
            for pin in range( 0, 16 ):
                self.set_mode( pin, mode = 'input' )  # , pullUp = 'enable' )

            # import RPi.GPIO as GPIO
            # GPIO.setwarnings( False )
            # GPIO.setmode( GPIO.BCM )

            # GPIO.setup( interruptPin, GPIO.IN, GPIO.PUD_DOWN )
            # GPIO.add_event_detect( interruptPin, GPIO.FALLING, self.callbackBoth, bouncetime = 50 )
            # self.interrupt_options( outputType = 'activelow', bankControl = 'both',  )
        else:
            for pin in range( 0, 16 ):
                self.output( pin, 1 )  # init as disabled
                self.set_mode( pin, mode = 'output' )

    def backup( self ):
        for address in range( 0x16 ):
            self.registry[ address ] = self.single_access_read( address )

    def restore( self ):
        for address in range( 0x16 ):
            self.single_access_write( address, self.registry[ address ] )
