var numpad = {
    ...
    init: function () {
        ...
        append("<i class='material-icons'>backspace</i>", numpad.delete, "ng");
        // Second row - 4 to 6, clear.
        for (var i = 4; i <= 6; i++) {
            append(i, numpad.digit);
        }
        append("<i class='material-icons'>clear</i>", numpad.reset, "ng");
        // Third row - 1 to 3, cancel.
        for (var i = 1; i <= 3; i++) {
            append(i, numpad.digit);
        }
        append("<i class='material-icons'>close</i>", numpad.hide, "cx");
        // Last row - 0, dot, ok
        append(0, numpad.digit, "zero");
        numpad.zero = button;
        append(".", numpad.dot);
        numpad.dot = button;
        append("<i class='material-icons'>check</i>", numpad.select, "ok");
    ...
    attach: function (opt) {
        ...
        if (target != null) {
            ...
            if (opt.readonly == undefined || typeof opt.readonly != "boolean") {
                opt.readonly = true;
            }
            if (opt.decimal == undefined || typeof opt.decimal != "boolean") {
                opt.decimal = true;
            }
